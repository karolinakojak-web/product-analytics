WITH users_no as (SELECT tenant_gid,
     ...
     tenants_stats AS (SELECT tenants.tenant_gid,
                              ...
                              if(active_subscription_plan_id in
                                 ('PLA-COR-2024', 'PLA-COR', 'PLA-PRE-2024', 'enterprise-2021', 'PLA-PRE',
                                  'enterprise'), 1, 0) as is_right_plan,
                              users_no.mau_avg,
     ...
     forms_submitted AS (SELECT tenant_gid,
                                actor_id,
                                user_type,
                         ...
                         WHERE event_type = 'form-submitted'
                           AND day >= '2026-01-01'
                           AND day < today()),
     tenant_forms AS (SELECT tenant_gid,
                      ...
                      GROUP BY tenant_gid),
     chats_dim as (select tenant_gid,
                          count(case when chat_type = 'GROUP' then 1 end) as group_chats,
                          count(case when chat_type = 'SMART' then 1 end) as smart_group_chats,
                   ...
                   GROUP BY tenant_gid),
     workflow_events AS (SELECT tenant_gid,
     ...
     tenant_workflows as (SELECT w.tenant_gid,
                                 countIf(JSONExtractString(JSONExtractArrayRaw(w.triggers)[1], 'id') ILIKE
                                         '%form%')                                                                 AS triggered_by_form_no,
                                 countIf(JSONExtractString(JSONExtractArrayRaw(w.triggers)[1], 'type') ILIKE
                                         '%SCHEDULE%')                                                             AS scheduled_workflows_no,
                                 count(*)                                                                          AS workflows_no,
                                 sum(we.workflow_executions_no)                                                    AS executions_no,
                                 sumIf(we.workflow_executions_no,
                                       JSONExtractString(JSONExtractArrayRaw(w.triggers)[1], 'id') ILIKE
                                       '%form%')                                                                   AS form_triggered_executions_no,
                                 sumIf(we.workflow_executions_no,
                                       JSONExtractString(JSONExtractArrayRaw(w.triggers)[1], 'type') ILIKE
                                       '%SCHEDULE%')                                                               AS scheduled_executions_no
     ...
     tenant_shifts as (SELECT tenant_gid,
                       ...
                       GROUP BY tenant_gid),
     tasks_events AS (SELECT tenant_gid,
                             event_type,
                             if(event_type = 'action-finished',
                                lower(JSONExtractString(event_properties_raw, 'step_id')),
                                '') AS step_id
                      FROM beekeeper_events
                      WHERE day > '2026-01-01'
                        AND event_type IN ('task-created', 'action-finished')),
     tasks AS (SELECT tenant_gid, count(*) AS created_tasks_no
               FROM tasks_events
               WHERE event_type = 'task-created'
               GROUP BY tenant_gid),
     tenant_tasks_workflows AS (SELECT tenant_gid, count(*) AS tasks_from_workflows_no
                                FROM tasks_events
                                WHERE event_type = 'action-finished'
                                  AND (startsWith(step_id, 'create_open_task') OR startsWith(step_id, 'create_task'))
                                GROUP BY tenant_gid),
     tenant_tasks AS (SELECT t.tenant_gid,
                             t.created_tasks_no,
                             tw.tasks_from_workflows_no,
                             round(tw.tasks_from_workflows_no / t.created_tasks_no, 2) AS tasks_from_workflows_pct
                      FROM tasks t
                               LEFT JOIN tenant_tasks_workflows tw ON tw.tenant_gid = t.tenant_gid)
SELECT tenants_stats.tenant_gid,
       subdomain,
       name,
       state,
       ...
       chats_dim.smart_group_chats,
       round(chats_dim.smart_group_chats / chats_dim.all_group_chats, 2) smart_group_chats_pct,
       ...
       tenant_tasks.created_tasks_no,
       tenant_tasks.tasks_from_workflows_no,
       tenant_tasks.tasks_from_workflows_pct
FROM tenants_stats
         ...
         LEFT JOIN tenant_tasks on tenants_stats.tenant_gid = tenant_tasks.tenant_gid;